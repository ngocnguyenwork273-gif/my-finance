...
import {
  ...
  Wallet, Landmark, Smartphone, X, Check, Loader2, Target,
} from 'lucide-react';

/* ---------- Dữ liệu tĩnh còn giữ tạm cho Report (sẽ nối sau) ---------- */
...
function Gauge({ limit, spent }) {
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    items.push(<line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={filled ? '#7c3aed' : '#ede9fe'} strokeWidth="4" strokeLinecap="round" />);
  }
  ...
  return (
    <svg viewBox="0 0 200 200" className="w-56 h-56">
      ...
      <foreignObject x={labelX - 34} y={labelY - 14} width="68" height="28">
        <div className="bg-gray-900 text-white text-[11px] font-medium rounded-full px-2 py-1 text-center whitespace-nowrap">{formatMoney(spent)}</div>
      </foreignObject>
          ...
          {pinnedItems.map((item) => {
            ...
            return (
              <div key={item.id} className="min-w-[150px] bg-white/90 backdrop-blur rounded-3xl p-4 shadow-lg shadow-black/5 flex-shrink-0">
                <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${item.color} flex items-center justify-center mb-6`}><Icon size={18} className="text-white" /></div>
          ...
          <div className="flex items-center gap-6">
            <svg width="150" height="150" viewBox="0 0 150 150" className="-rotate-90 flex-shrink-0">
              {categoryColors.map((cat) => {
                ...
                cumulative += dash;
                return <circle key={cat.name} cx="75" cy="75" r={radius} fill="none" stroke={cat.color} strokeWidth="14" strokeDasharray={`${dash} ${circumference - dash}`} strokeDashoffset={-offset} strokeLinecap="round" />;
              })}
            </svg>
              ...
              {transactions.map((tx) => {
                ...
                return (
                  <div key={tx.id} className="flex items-center gap-3 py-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${tx.type === 'income' ? 'bg-emerald-50' : 'bg-violet-50'}`}><Icon size={18} className={tx.type === 'income' ? 'text-emerald-600' : 'text-violet-600'} /></div>
      ...
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        ...
        <div className="mt-6 bg-white rounded-t-[2.5rem] min-h-[45vh] px-5 pt-6 pb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {periods.map((p) => (
              <button key={p} onClick={() => setPeriod(p)} className={`px-4 py-1.5 rounded-full text-sm flex-shrink-0 ${period === p ? 'bg-gray-900 text-white font-medium' : 'bg-gray-100 text-gray-500'}`}>{periodLabels[p]}</button>
            ))}
          </div>
          <div className="flex flex-col divide-y divide-gray-100 mt-4">
            {reportTransactions.map((tx) => {
              ...
              return (
                <div key={tx.id} className="flex items-center gap-3 py-3">
                  <div className="w-10 h-10 rounded-full bg-violet-50 flex items-center justify-center flex-shrink-0"><Icon size={18} className="text-violet-600" /></div>
                  <div className="flex-1 min-w-0">
...
function AddGoalForm({ onClose, onSaved }) {
  ...
  async function handleSave() {
    if (!name || !target) {
      alert('Nhập đủ tên mục tiêu và số tiền mục tiêu');
      return;
    }
    setSaving(true);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end z-20" onClick={onClose}>
      <div className="bg-white w-full rounded-t-3xl p-5 max-w-sm mx-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">Mục tiêu mới</h3>
          <button onClick={onClose}><X size={18} className="text-gray-500" /></button>
        </div>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Tên mục tiêu (vd: Mua xe)" className="w-full bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none mb-3" />
...
/* ---------- Form nhỏ: Đặt hạn mức cho 1 category ---------- */

function AddBudgetForm({ expenseCategories, onClose, onSaved }) {
  ...
  async function handleSave() {
    if (!categoryId || !limit) {
      alert('Chọn danh mục và nhập hạn mức');
      return;
    }
    setSaving(true);
    const { error } = await supabase.from('categories').update({ monthly_limit: Number(limit) }).eq('id', categoryId);
    setSaving(false);
    if (error) { alert('Lỗi: ' + error.message); return; }
    onSaved();
    onClose();
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end z-20" onClick={onClose}>
      <div className="bg-white w-full rounded-t-3xl p-5 max-w-sm mx-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">Đặt hạn mức chi tiêu</h3>
          <button onClick={onClose}><X size={18} className="text-gray-500" /></button>
        </div>
        <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none mb-3">
          <option value="">-- Chọn danh mục chi tiêu --</option>
          {expenseCategories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <input value={limit} onChange={(e) => setLimit(e.target.value.replace(/\D/g, ''))} inputMode="numeric" placeholder="Hạn mức mỗi tháng" className="w-full bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none mb-4" />
        <button onClick={handleSave} disabled={saving} className="w-full bg-gray-900 text-white rounded-xl py-3 font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
          {saving ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} Lưu hạn mức
        </button>
      </div>
    </div>
  );
...
/* ---------- Màn Goals & Budget (dữ liệu THẬT từ Supabase) ---------- */

function Goals({ setScreen, goals, categories, transactions, loadingGoals, reload }) {
  const [showAddGoal, setShowAddGoal] = useState(false);
  const [showAddBudget, setShowAddBudget] = useState(false);

  const expenseCategories = categories.filter((c) => c.type === 'expense');
  const budgetCategories = expenseCategories.filter((c) => c.monthly_limit);
  ...
  function spentThisMonth(categoryId) {
    return transactions
      .filter((tx) => {
        ...
        return tx.category_id === categoryId && tx.type === 'expense' && d.getMonth() === currentMonth && d.getFullYear() === currentYear;
      })
      ...
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        ...
        <div className="mt-6 bg-white rounded-t-[2.5rem] min-h-[80vh] px-5 pt-6 pb-6">
          ...
          {loadingGoals ? (
            ...
            <p className="text-gray-400 text-sm text-center py-6">Chưa có mục tiêu nào. Bấm + để tạo.</p>
          ) : (
                  ...
                  <div key={goal.id}>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-900 font-semibold text-sm">{formatMoney(goal.current_amount || 0)}</p>
                      </div>
                    </div>
                    <ProgressBar pct={pct} />
                    <div className="flex justify-between mt-1 text-xs text-gray-400">
                      <span>{formatMoney(goal.current_amount || 0)}</span>
          ...
          <div className="flex items-center justify-between mt-8 mb-3">
            <h2 className="text-gray-900 font-semibold text-lg">Ngân sách</h2>
            <button onClick={() => setShowAddBudget(true)} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center"><Plus size={16} className="text-gray-600" /></button>
          </div>

          {budgetCategories.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6">Chưa đặt hạn mức nào. Bấm + để thêm.</p>
          ) : (
            <div className="flex flex-col gap-5">
              {budgetCategories.map((cat) => {
                const Icon = getIcon(cat.icon);
                const spent = spentThisMonth(cat.id);
                const pct = (spent / cat.monthly_limit) * 100;
                const remaining = cat.monthly_limit - spent;
                return (
                  <div key={cat.id}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0"><Icon size={18} className="text-gray-600" /></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-gray-900 font-medium text-sm">{cat.name}</p>
                        <p className="text-gray-900 font-semibold text-sm">{formatMoney(cat.monthly_limit)}</p>
                      </div>
                    </div>
                    <ProgressBar pct={pct} colorClass={pct > 100 ? 'bg-red-400' : 'bg-orange-300'} />
                    <div className="flex justify-between mt-1 text-xs text-gray-400">
                      <span>{remaining >= 0 ? `Còn lại khoảng ${formatMoney(remaining)}` : `Vượt ${formatMoney(remaining)}`}</span>
                      <span>{formatMoney(cat.monthly_limit)}</span>
                    </div>
                  </div>
                );
              })}
        ...
        {showAddBudget && <AddBudgetForm expenseCategories={expenseCategories} onClose={() => setShowAddBudget(false)} onSaved={reload} />}
...
function AddTransaction({ setScreen, accounts, categories, onSaved }) {
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from('transactions').insert({
      account_id: selectedAccount, category_id: selectedCategory, type, amount: Number(amount), note: note || null,
    });
    setSaving(false);
      ...
      <div className="w-full max-w-sm min-h-screen pb-10 relative">
        ...
        <div className="px-5 mt-8 text-center">
          ...
          <div className="flex items-center justify-center gap-1">
            <input type="text" inputMode="numeric" value={amount ? Number(amount).toLocaleString('vi-VN') : ''} onChange={handleAmountChange} placeholder="0" className={`text-4xl font-bold text-center bg-transparent outline-none w-full ${type === 'income' ? 'text-emerald-600' : 'text-gray-900'}`} />
        ...
        <div className="px-5 mt-8">
          ...
          {categoryList.length === 0 ? <p className="text-gray-400 text-sm">Chưa có danh mục nào cho loại này.</p> : (
              ...
              {categoryList.map((cat) => {
                ...
                return (
                  <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className="flex flex-col items-center gap-1.5">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition" style={{ background: active ? (cat.color || '#7c3aed') : '#f3f4f6' }}><Icon size={20} className={active ? 'text-white' : 'text-gray-500'} /></div>
        ...
        <div className="px-5 mt-8">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {accounts.map((acc) => {
              ...
              return (
                <button key={acc.id} onClick={() => setSelectedAccount(acc.id)} className={`flex items-center gap-2 px-3 py-2 rounded-full flex-shrink-0 border transition ${active ? 'bg-gray-900 border-gray-900 text-white' : 'bg-white border-gray-200 text-gray-600'}`}>
                  <Icon size={16} /><span className="text-sm">{acc.name}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="px-5 mt-8">
          ...
          <input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Thêm ghi chú (không bắt buộc)" className="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm outline-none" />
        </div>
        <div className="px-5 mt-10">
          <button onClick={handleSave} disabled={saving} className="w-full bg-gray-900 text-white rounded-2xl py-4 font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
            {saving ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}{saving ? 'Đang lưu...' : 'Lưu giao dịch'}
          </button>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  if (screen === 'goals') return <Goals setScreen={setScreen} goals={goals} categories={categories} transactions={transactions} loadingGoals={loadingGoals} reload={loadAll} />;
