...
import {
  ...
  ArrowLeft, Download, X, Check, Loader2, Target, Wallet, Trash2, Pencil,
} from 'lucide-react';
...
// Vòng tròn hiện emoji trực tiếp (không cần map sang icon code)
function EmojiCircle({ emoji, size = 36, active = false, activeColor = '#7c3aed', bg = '#f3f4f6' }) {
  return (
    <div className="rounded-xl flex items-center justify-center flex-shrink-0"
      style={{ width: size, height: size, background: active ? activeColor : bg, fontSize: size * 0.5 }}>
      {emoji || '❔'}
    </div>
  );
}
...
function Dashboard({ setScreen, transactions, categories, loading }) {
  ...
  const spentByCat = expenseCats.map((c) => ({
    ...c,
    amount: transactions.filter((t) => t.category_id === c.id && t.type === 'expense').reduce((s, t) => s + Number(t.amount), 0),
  })).filter((c) => c.amount > 0);
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-400 via-fuchsia-300 to-orange-100 flex justify-center">
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        <div className="px-5 pt-8 flex items-center justify-between">
          ...
          <button onClick={() => setScreen('accounts')} className="w-11 h-11 rounded-full bg-white/30 backdrop-blur flex items-center justify-center text-white border border-white/40">
            <Wallet size={18} />
          </button>
        </div>
        ...
        <div className="mt-6 px-5 flex gap-3 overflow-x-auto pb-2">
          {fundCategories.length === 0 ? (
            <p className="text-white/70 text-sm">Đánh dấu danh mục là "Quỹ" trong Cài đặt để hiện ở đây.</p>
        ...
        <div className="mt-6 bg-white rounded-t-[2.5rem] min-h-[60vh] px-5 pt-6 pb-6">
          ...
          {spentByCat.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6">Chưa có chi tiêu nào tháng này.</p>
          ) : (
              ...
              <div className="flex flex-col gap-2 text-sm min-w-0">
                {spentByCat.map((cat, i) => (
                  <div key={cat.id} className="flex items-center gap-2">
                    ...
                    <span className="text-gray-900 font-medium ml-auto">{formatMoney(cat.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
...
/* ---------- Report (chưa nối dữ liệu thật, giữ giao diện gauge) ---------- */
      ...
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        ...
        <div className="mt-6 bg-white rounded-t-[2.5rem] min-h-[45vh] px-5 pt-6 pb-6">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {periods.map((p) => <button key={p} onClick={() => setPeriod(p)} className={`px-4 py-1.5 rounded-full text-sm flex-shrink-0 ${period === p ? 'bg-gray-900 text-white font-medium' : 'bg-gray-100 text-gray-500'}`}>{periodLabels[p]}</button>)}
          </div>
      ...
      <div className="w-full max-w-sm min-h-screen pb-10 relative">
        ...
        <div className="px-5 mt-8 text-center">
          ...
          <div className="flex items-center justify-center gap-1">
            <input type="text" inputMode="numeric" value={amount ? Number(amount).toLocaleString('en-US') : ''} onChange={handleAmountChange} placeholder="0"
              className={`text-4xl font-bold text-center bg-transparent outline-none w-full ${overLimit ? 'text-red-500' : type === 'income' ? 'text-emerald-600' : 'text-gray-900'}`} />
            ...
            {accounts.map((acc) => {
              ...
              return (
                <button key={acc.id} onClick={() => setSelectedAccount(acc.id)} className={`flex items-center gap-2 px-3 py-2 rounded-full flex-shrink-0 border transition ${active ? 'bg-gray-900 border-gray-900 text-white' : 'bg-white border-gray-200 text-gray-600'}`}>
                  <span>{acc.icon}</span><span className="text-sm">{acc.name}</span>
                </button>
              );
            })}
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-400 via-fuchsia-200 to-orange-100 flex justify-center">
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        ...
        <div className="px-5 mt-4 text-center">
          <p className="text-white/80 text-sm">Tổng tất cả tài khoản</p>
            ...
            {accounts.map((acc) => (
              <div key={acc.id} className="flex items-center gap-3 bg-gray-50 rounded-2xl p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-400 text-xs capitalize">{acc.type}</p>
                </div>
...
/* ---------- Cài đặt: quản lý danh mục (thêm/sửa/xóa, không cần code) ---------- */

function Settings({ setScreen, categories, reload }) {
  const [tab, setTab] = useState('expense');
  const [editing, setEditing] = useState(null); // category object đang sửa, hoặc 'new'
  const [form, setForm] = useState({ name: '', icon: '', monthly_limit: '', is_fund: false });
  const [saving, setSaving] = useState(false);

  function startNew() {
    setForm({ name: '', icon: '', monthly_limit: '', is_fund: false });
    setEditing('new');
  }
  function startEdit(cat) {
    setForm({ name: cat.name, icon: cat.icon || '', monthly_limit: cat.monthly_limit || '', is_fund: cat.is_fund || false });
    setEditing(cat.id);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const payload = {
      name: form.name, icon: form.icon || '❔', type: tab,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-400 via-fuchsia-200 to-orange-100 flex justify-center">
      <div className="w-full max-w-sm min-h-screen pb-28 relative">
        <div className="px-5 pt-8 flex items-center gap-3">
          <button onClick={() => setScreen('dashboard')} className="w-9 h-9 rounded-full bg-white/30 backdrop-blur flex items-center justify-center"><ArrowLeft size={18} className="text-white" /></button>
          <h1 className="text-white text-lg font-semibold">Cài đặt danh mục</h1>
        </div>
        ...
        <div className="mt-6 bg-white rounded-t-[2.5rem] min-h-[80vh] px-5 pt-6 pb-6">
          ...
          <button onClick={startNew} className="w-full border-2 border-dashed border-gray-200 rounded-2xl py-3 text-sm text-gray-500 font-medium mb-4 flex items-center justify-center gap-2">
        ...
        {editing && (
          <div className="fixed inset-0 bg-black/40 flex items-end z-20" onClick={() => setEditing(null)}>
            <div className="bg-white w-full rounded-t-3xl p-5 max-w-sm mx-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-gray-900">{editing === 'new' ? 'Danh mục mới' : 'Sửa danh mục'}</h3>
                <button onClick={() => setEditing(null)}><X size={18} className="text-gray-500" /></button>
              </div>
              ...
              <label className="flex items-center gap-2 mb-4 text-sm text-gray-600">
                <input type="checkbox" checked={form.is_fund} onChange={(e) => setForm({ ...form, is_fund: e.target.checked })} />
                Đây là 1 "quỹ" — hiện thẻ tổng tiền ở Trang chủ
              </label>
